...
class SignupRVAdapter (private val items: ArrayList<UserRepoData>) : RecyclerView.Adapter<SignupRVAdapter.Holder>() {
    inner class Holder(itemView: View?):RecyclerView.ViewHolder(itemView!!){
        ...
        fun bind(position: Int){
            num?.text = "${position + 1}"
            nameRepository?.text = items[position].nameRepository
